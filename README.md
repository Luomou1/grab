...
HTGE 相机 + PZT 位移台的纯 Python 采集程序，提供桌面 GUI、软触发扫描、ROI 框选、暗场/平场校正和图像保存能力，目标是替代原有 MATLAB/厂商工具流程，统一相机采集与 PZT 控制。
...
- ROI 框选与参数回填，可从预览窗口直接选择传感器区域。
- 普通扫描 / 中心扫描两种模式，支持软触发和连续采集。
- 暗场 / 平场校正，扫描时可输出校正结果和原始图像。
- 纯 Python 测试，覆盖 PZT 协议、图像保存、ROI、扫描异步保存等关键逻辑。
